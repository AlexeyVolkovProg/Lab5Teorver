...
public class DataAnalyzer {
    ...
    public void doAnalyze(ArrayList<Double> variation){
        System.out.println("Диапазон вариации:");
        sortArray(variation);
        System.out.println(variation);
        System.out.println("Крайние значения вариации:");
        System.out.println("Минимальное = " + min(variation) + " ; Максимальное = " + max(variation));
        System.out.println("Диапазон изменения  = " + range(variation));
        System.out.println("Мат. ожидание = " + expectation(variation));
        System.out.println("Среднеквадратическое отклонение = " + Math.sqrt(variationDispersion(variation)));
    ...
    /**
     * Сортируем по возрастанию(основываясь на самих значениях)
     */
    ...
    /**
     * Ищем минимальное значение в выборке
     */
    ...
    /**
     * Ищем максимальное зн-ие в выборке
     */
    ...
    /**
     * Считаем диапазон изменения(максимально - минимальное)
     */
    ...
    /**
     *Позволяет по методы Стерджеса определить оптимальное кол-во интервалов для эмпирической функции
     */
    ...
    /**
     * Ищем интервальные значения для эмпирической функции с учетом оптимального их кол-ва Методом Стерджеса
     */
    public ArrayList<Double> intervalBorders(ArrayList<Double> variation){
        ...
        Double intervalWidth = range(variation) / intervalsCount; // метод Стерджеса
    ...
    public ArrayList<Double> intervalMiddles(ArrayList<Double> borders){
        ...
        for(int i = 0; i < borders.size() -1; i++){
            middles.add((borders.get(i+1) + borders.get(i)) / 2);
        }
        return middles;
    }
    ...
    /**
     * Подсчет кол-ва чисел(частоты появления) на нужных нам интервалах.
     * Нужна для Эмпирической функции
     */
    public ArrayList<Long> accurateCumulativeFrequency(ArrayList<Double> variation, ArrayList<Double> borders){
        ...
        for(int i = 0; i < borders.size() - 1; i++){
            ...
            //ищем только те значения, которые находятся в интервале v>= borders.get(finalI) и v < borders.get(finalI + 1)
        }
        return frequencies;
    }
    ...
    /**
     * Реализация эмпирической функции
     */
